...
p=Path(sys.argv[sys.argv.index('--')+1]);bpy.ops.wm.open_mainfile(filepath=str(p/'pilgrim_devotional.blend'));scene=bpy.data.scenes['PILGRIM_Devotional_Loop'];bpy.context.window.scene=scene;rig=bpy.data.objects[scene['rig']]
d=json.loads((p/'skeleton.json').read_text());expected=to_blender(np.load(p/'motion.npz')['clean']);names=d['names'];ix={n:i for i,n in enumerate(names)};prefix=rig.name[:-4];ctrl=rig['control_prefix'];control_action=bpy.data.actions[scene['control_action']];baked_action=bpy.data.actions[scene['baked_action']]
meshes=[o for o in bpy.data.collections['PILGRIM_SETTLE_BODY'].objects if o.type=='MESH'];bones={o.name:o.vertex_groups[0].name for o in meshes}
mounts=set()
for key in 'ABCD':
...
report=dict(control_integer_joint_error=0.,bake_vs_controls_max_error=0.,min_mesh_z=1e9,rear_contact_drift=0.,surface_intersections=[],tested_subframes=481)
controls=[]
for f in np.linspace(1,121,481):
...
at(1);start=xyz();q1=head_q();at(121);end=xyz();q2=head_q();report['seam_position_error']=float(np.linalg.norm(end-start,axis=-1).max());report['seam_head_angle_radians']=float(q1.rotation_difference(q2).angle)
h=.05
at(1+h);right=xyz();at(121-h);left=xyz();report['seam_velocity_mismatch_units_per_second']=float(np.linalg.norm(((right-start)-(end-left))*20/h,axis=-1).max())
rig.animation_data.action=baked_action;rig['use_controls']=0.;rig.update_tag()
for f,reference in zip(np.linspace(1,121,481),controls):
