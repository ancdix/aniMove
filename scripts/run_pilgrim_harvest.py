"""Unguided three-second motion harvest on Pilgrim, with matched Hound controls."""
...
def main():
    p=argparse.ArgumentParser();p.add_argument('condition',type=Path);p.add_argument('--name',default='motion_harvest_001');p.add_argument('--families',nargs='+',default=['all','quadropeds','bipeds']);p.add_argument('--seeds',type=int,nargs='+',default=list(range(7100,7140)));cli=p.parse_args()
    ...
    run=dict(status='running',condition_manifest=str(cli.condition/'condition_manifest.json'),condition_sha256=sha256(cm['condition']),
             ...
             calibration_caveat=cm['caveat'],fps=20,frames=60,seeds=cli.seeds,clips=[],checkpoints={},source_revision=subprocess.check_output(['git','rev-parse','HEAD'],text=True).strip(),
    ...
    run['generator_sha256']=sha256(ROOT/'scripts/run_pilgrim_harvest.py');run['sampling_note']='120 independent unguided 60-frame samples, plus 3 matched Hound controls. No action labels, keyframes or authored poses.'
    import shutil
    shutil.copy2(ROOT/'scripts/run_pilgrim_harvest.py',out/'generator_snapshot.py');shutil.copy2(Path(cm['skeleton']),out/'skeleton.json')
    save();t5=None
    for family in cli.families:
        ...
        sys.argv=['generate','--model_path',str(checkpoint),'--object_type','Pilgrim','--cond_path',cm['condition'],'--motion_length','3']
        ...
        for object_type,seeds in [('Pilgrim',cli.seeds),('Hound',cli.seeds[:1])]:
            c=cond[object_type];_,kwargs=create_condition([object_type],cond,60,args.temporal_window,t5,opt.max_joints,opt.feature_len)
            for seed in seeds:
                fixseed(seed);start=time.monotonic()
                with torch.no_grad():sample=diffusion.p_sample_loop(model,(1,opt.max_joints,model.feature_len,60),clip_denoised=False,model_kwargs=kwargs,skip_timesteps=0,init_image=None,progress=False)
                ...
                run['clips'].append(entry);save();print('HARVEST_SAMPLE',json.dumps({k:v for k,v in entry.items() if k not in ['parents','names']}),flush=True)
        del model,diffusion;gc.collect();torch.cuda.empty_cache()
    run['status']='generated';save();print('HARVEST_GENERATION_COMPLETE',out,flush=True)
