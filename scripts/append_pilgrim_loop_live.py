"""Append only the verified devotional asset and comparison; preserve live work."""
import bpy,json
from pathlib import Path
p=Path('/media/ipsedesktop/ShareDrive1/ModelData/aniMove/pilgrim_machine/devotional_loop_v003')
assert json.loads((p/'saved_validation.json').read_text())['status']=='passed'
assert json.loads((p/'package_validation.json').read_text())['status']=='passed'
scene_names=['PILGRIM_Devotional_Hold','PILGRIM_Devotional_Loop'];baked='PILGRIM_Devotional_Final_FK_v001'
assert not any(n in bpy.data.scenes for n in scene_names),'Devotional scene already loaded; do not duplicate'
...
with bpy.data.libraries.load(str(p/'pilgrim_devotional_master.blend'),link=False) as (source,target):
    assert all(n in source.scenes for n in scene_names);assert baked in source.actions
    target.scenes=list(scene_names);target.actions=[baked];target.texts=[n for n in source.texts if n in ['README_PILGRIM_DEVOTIONAL','PILGRIM_CONTROL_UTILITIES.py']]
scene=bpy.data.scenes['PILGRIM_Devotional_Hold'];bpy.context.window.scene=scene;scene.frame_set(1);scene.sync_mode='FRAME_DROP';rig=bpy.data.objects[scene['rig']]
