"""Append verified guided comparisons without replacing existing live work."""
import json
from pathlib import Path
import bpy

out = Path('/media/ipsedesktop/ShareDrive1/ModelData/aniMove/unimate/pilgrim_guided_001')
...
with bpy.data.libraries.load(str(out / 'guided_review.blend'), link=False) as (source, target):
    target.scenes = [name]
    target.actions = [a for a in source.actions if a.startswith(('UNI_probe_words_', 'UNI_probe_guided_'))]
    target.texts = ['UNIMATE_GUIDED_UI.py', 'README_UNIMATE_GUIDED']
s = bpy.data.scenes[name]
bpy.context.window.scene = s
s.sync_mode = 'FRAME_DROP'
ns = {'__name__': 'unimate_guided_live'}
exec(bpy.data.texts['UNIMATE_GUIDED_UI.py'].as_string(), ns)
bpy.app.driver_namespace['unimate_guided_ui'] = ns
for case in ['words_9700', 'guidance_9700', 'words_9701', 'words_9700']:
    s.unimate_guided_case = case
